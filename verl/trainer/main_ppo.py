...
def run_ppo(config, task_runner_class=None) -> None:
    ...
    """TaskRunner 包成一个 Ray actor class"""
    ...
    """在远程 TaskRunner actor 上执行 run(config),真实训练是在TaskRunner.run(config)"""
...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        """判断用不用 lora，来判断在后面加载 actor worker 的时候需不需要加载 ref model 的参数"""
        ...
        """创建 ActorRollout 角色，就用 ActorRolloutRefWorker 这个类创建 Ray actor。"""
    ...
    def init_resource_pool_mgr(self, config):
        ...
        """既然开了 distillation，就必须给 teacher 分 GPU 和节点"""
    ...
    def add_teacher_model_resource_pool(self, config):
        ...
        from verl.trainer.ppo.ray_trainer import Role
        """作用只是进行一个resource pool 的 mapping"""
    ...
    def run(self, config):
        ...
        """
        通过 config 去判断是否使用 ref mdoel（比如用了 lora 就不用额外的 ref 了），并且登记了对应的 worker 和  resource pool 的关系
        返回的这俩和 config 没关系。
        """
        actor_rollout_cls, ray_worker_group_cls = self.add_actor_rollout_worker(config)

        """opd流程critic和reward这俩不需要"""
        self.add_critic_worker(config)
        self.add_reward_model_resource_pool(config)

        """要是启动 opd的话，在 resource pool 中对teacher model 分类资源在teacher_pool中"""
        self.add_teacher_model_resource_pool(config)

        # Add a reference policy worker if KL loss or KL reward is used.
        """reference policy 被合并到了 ActorRolloutRefWorker 里面。这部分代码只是为了兼容旧的代码"""
        ...
        """student/actor 模型在当前机器上可以被读取的路径。"""
        ...
        """从之前的 local path 中加载出来tokenizer"""
